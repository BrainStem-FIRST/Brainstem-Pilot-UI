...
function NumberField({ label, value, onChange, unit }) {
  return (
    <label className="flex flex-col gap-1 min-w-0">
      <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">{label}{unit ? ` (${unit})` : ''}</span>
      <input type="number" step={0.01} value={Number.isFinite(value) ? parseFloat(value.toFixed(3)) : 0}
...
function RecordCard({ accent, thumbnail, name, onRename, meta, children, usage, onDelete }) {
  return (
    <div className={`rounded-xl bg-card border ${accent} overflow-hidden flex flex-col group`}>
      <div className="relative aspect-[2/1] bg-[#0d1117] border-b border-border/60 overflow-hidden">
        {thumbnail}
      </div>
...
export default function LibraryIndex() {
  ...
  const { record, undo, redo, canUndo, canRedo } = useUndoRedo({ getSnapshot, applySnapshot });
  ...
  const updatePoint = (rec, updates) =>
    mutate(lib => ({ ...lib, points: lib.points.map(p => p.id === rec.id ? { ...p, ...updates } : p) }));
                  ...
                  {kind === 'path' ? (
                    ...
                    >
                      <div className="grid grid-cols-3 gap-2">
                        <NumberField label="X" unit={unit} value={rec.x ?? 0} onChange={v => updatePoint(rec, { x: v })} />
                        <NumberField label="Y" unit={unit} value={rec.y ?? 0} onChange={v => updatePoint(rec, { y: v })} />
                        <NumberField label="Rot" unit="°" value={rec.rotation ?? 0} onChange={v => updatePoint(rec, { rotation: v })} />
                      </div>
                    </RecordCard>
                  )}
                </motion.div>
